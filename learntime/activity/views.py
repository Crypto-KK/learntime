...
class ActivityCreateByAcademy(RoleRequiredMixin, CreateView):
    """学院辅导员发布活动"""
    role_required = (RoleEnum.ACADEMY.value, )
    model = Activity
    template_name = "activity/activity_create_by_academy.html"
    form_class = ActivityForm

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['schools'] = get_user_model().objects.filter(role=2)
        return context
    ...
    def form_valid(self, form):
        ...
        if form.instance.scope == self.request.user.academy:
            ...
            to_school = get_user_model().objects.get(pk=self.request.POST['to'])
            form.instance.to_school = to_school
        form.instance.user = self.request.user
        form.instance.is_craft = False #
        # 发送异步邮件给指定的审核者 to_admin
        # send_activity_verify_email(to_admin.email)
